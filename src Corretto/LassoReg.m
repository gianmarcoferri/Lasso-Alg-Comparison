    ...
    methods
        ...
        function ista(obj)
            for i = 1:obj.max_iterations
                ...
                % Soft-thresholding
                new_W = obj.soft_threshold(new_W_unthresholded, ...
                                           obj.step_size * obj.l1_penalty);
        ...
        function admm(obj)
            ...
            for i = 1:obj.max_iterations
                ...
                tol_prim = sqrt(obj.n) * abs_tol + ...
                           rel_tol * max(norm(obj.W), norm(z));
                tol_dual = sqrt(obj.n) * abs_tol + ...
                           rel_tol * norm(rho * u);
        ...
        function distributed_admm(obj, agents)
            ...
            for i = 1:obj.max_iterations
                ...
                for j = 1:agents
                    ...
                    w_temp = (X_j' * X_j + rho * I) \ ...
                              (X_j' * Y_j + rho * (z - u_agents(j,:))');
                    W_agents(j,:) = w_temp';
                end
                
                % Aggiornamento del consenso z
                z = obj.soft_threshold(mean(W_agents + u_agents, 1), ...
                                       obj.l1_penalty / (rho * agents));
                ...
                s_norm = norm(-rho * (z - last_z));
                tol_prim = sqrt(obj.n) * abs_tol + ...
                           rel_tol * max(norm(mean(W_agents, 1)), norm(z));
                tol_dual = sqrt(obj.n) * abs_tol + ...
                           rel_tol * norm(rho * mean(u_agents, 1));
        ...
        function loss = loss_function(obj, Y, Y_predict, W)
            loss = (1/2 * sum((Y - Y_predict).^2) + ...
                    obj.l1_penalty * norm(W, 1));
        end
